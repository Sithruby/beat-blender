...
export default function SignUp({ props }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {username.length < 5 ? null : !nameVerify && (
        <Text style={{ marginLeft: 20, color: 'rgb(149, 0, 0)' ,paddingBottom:12}}>
          Name should be more than 5 character
        </Text>
      )}
      <View style={styles.inputContainer}>
        <TextInput
      ...
      {email.length < 1 ? null : !emailVerify && (
        <Text style={{ color: 'rgb(149, 0, 0)' ,paddingBottom:12}}>
      ...
      <View style={styles.inputContainer}>
        ...
        {password.length > 0 && (
          <Feather
            name={pwVerify ? 'check-circle' : 'x-circle'}
            color={pwVerify ? 'green' : 'rgb(139, 0, 0)'}
            size={20}
            style={styles.icon}
          />
        )}
      </View>
      {password.length < 6 ? null : !pwVerify && (
        <Text style={{ marginLeft: 20, color: 'rgb(149, 0, 0)',paddingBottom:12 }}>
          Password should be more than 6 character
        </Text>
      )}
      
      <TouchableOpacity style={styles.button} onPress={() => registerUser(username, email, password)}>
